...
import { isUserInProject, getProjectIdFromReceipt, isUserProjectOwner } from '../auth/auth.service.js';
...
export async function projectOwnerMiddleware(req: ProjectRequest, res: Response, next: NextFunction) {
    try {
        const userId = req.user!.userId;
        const { projectId } = req.params;
